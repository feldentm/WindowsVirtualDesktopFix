			...
			,&Offset_Right:="",&Offset_Bottom:="")
{
    ...
    RECTPlus := Buffer(32, 0) ; V1toV2: if 'RECTPlus' is a UTF-16 string, use 'VarSetStrCapacity(&RECTPlus, 32)'
...
ShowWinGetPosEx(hWindow) {
	WinGetPosEx(hWindow, &X, &Y, &W, &H, &OL, &OT, &ORI, &OB)
	MsgBox "Window is at " X "," Y " and its size is " W "x" H " with offset " OL ", " OT ", " ORI ", " OB
	return
}

WinGetOffsets(Window,&Offset_Left:="",&Offset_Top:="",&Offset_Right:="",&Offset_Bottom:="") {
	HWND := WinGetID("A")
	WinGetPosEx(HWND, , , , , &Offset_Left, &Offset_Top, &Offset_Right, &Offset_Bottom)
}